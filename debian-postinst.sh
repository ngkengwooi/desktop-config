...
if [ $EUID -eq 0 ]; then
  ...
  #############################
  sed -i 's/^\s*GRUB_TIMEOUT=\d+\s*$/GRUB_TIMEOUT=0/' /etc/default/grub
  sed -i 's/^\s*GRUB_CMDLINE_LINUX_DEFAULT="[^"]+"\s*$/GRUB_CMDLINE_LINUX_DEFAULT="splash quiet"/' /etc/default/grub
  update-grub2
